...
from trl import AutoModelForCausalLMWithValueHead, CPOConfig, CPOTrainer  # Changed from ORPO to CPO
import nltk
from nltk.tokenize import word_tokenize
import difflib
import wandb  # Add wandb import for explicit control
...
class IterativeCPOTrainer:  # Changed class name from IterativeORPOTrainer
    def __init__(self, 
    ...
    def train_cpo_iteration(self, iteration: int, train_data: List[Dict]):  # Changed method name
        """Train CPO model for one iteration"""
        print(f"Starting CPO training iteration {iteration}")  # Updated message
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype=torch.float32,      # Use float32 to prevent NaN issues
        ...
        training_args = CPOConfig(  # Changed from ORPOConfig to CPOConfig
            output_dir=iteration_output_dir,
            **training_config_copy  # Now run_name won't be overridden
        )
        
        trainer = CPOTrainer(  # Changed from ORPOTrainer to CPOTrainer
            model=model, 
            args=training_args, 
        ...
        del model
        del trainer  
        del hf_dataset  # Also cleanup the dataset
        torch.cuda.empty_cache()
    ...
    def run_iterative_training(self, num_iterations: int = 5):
        ...
        print(f"Starting iterative CPO training with {num_iterations} iterations (FIXED SIZE - NO OVERFITTING)")  # Updated message
        ...
        for iteration in range(num_iterations):
            ...
            # FIXED SIZE APPROACH: Store all data but train only on current batch
            self.all_training_data.extend(batch_data)  # Keep for final JSON saving
            ...
            # 🔑 KEY CHANGE: Train CPO on ONLY current batch data (avoid overfitting)
            
            # Ensure wandb creates a new run for this iteration
            try:
                wandb.finish()  # End any existing run
            except:
                pass  # Ignore if no run exists
            
            print(f"🔄 Reinitializing wandb for iteration {iteration + 1}")
            
            trained_model_path = self.train_cpo_iteration(iteration + 1, batch_data)  # Updated method call
        ...
        print(f"Each iteration trained on ~{len(self.all_training_data)//num_iterations} samples")
        print(f"Final training data saved to: {self.final_json_path}")

def main():
    # Ensure wandb is properly configured
    os.environ["WANDB_PROJECT"] = "iterative-cpo-training"
    os.environ["WANDB_LOG_MODEL"] = "false"  # Don't log model artifacts to save space
    
    print(f"🔧 Wandb project: {os.environ.get('WANDB_PROJECT')}")
    print(f"🔧 Wandb enabled: {os.environ.get('WANDB_DISABLED', 'false') != 'true'}")
    
    # Model Configuration - Updated for OPT-1024
    model_config = {
        "initial_child_model_name": "babylm-seqlen/opt-4096-warmup-v2",  # Updated to match ORPO version
        "child_tokenizer_name": "babylm-seqlen/tokenizer",  # Updated to match ORPO version
        "teacher_model_name": "meta-llama/Llama-3.2-3B-Instruct",
    }
    
    # Output Configuration
    output_config = {
        "output_dir": "iterative_cpo_training_3b_results_babylm_opt_seqlen_4096_stable_config",  # Updated naming
        "final_json_path": "final_iterative_training_cpo_3b_data_babylm_opt_seqlen_4096_stable_config.json",  # Updated naming
        "num_iterations": 5
    }
    ...
    # CPO Training Configuration (using stable settings like ORPO)
    training_config = {
        "logging_steps": 10,
        "per_device_train_batch_size": 1,  # Reduced to prevent NaN
        "gradient_accumulation_steps": 8,  # Keep effective batch size
        "gradient_checkpointing": True,    # Trade compute for memory
        "dataloader_drop_last": True,
        "fp16": False,                     # Disable mixed precision to prevent NaN
        "learning_rate": 1e-6,             # Reduced for stability
        "num_train_epochs": 1,             # Single epoch per iteration
        "save_steps": 500,
        "eval_steps": 500,
        "warmup_steps": 10,
        "max_grad_norm": 0.5,              # Reduced gradient clipping for stability
    ...
    trainer = IterativeCPOTrainer(  # Updated class name
